...
template <class TileShape_MNK_, class Element_, int NumEpilogueThreads_,
          bool Varlen_>
struct CollectiveEpilogueBwd {
  ...
  static_assert(get<2>(TileShape_MNK{}) % kGmemElemsPerLoad == 0,
  ...
  using SmemLayoutdKVTMA = decltype(tile_to_shape(
      SmemLayoutAtomdKVTMA{}, select<1, 2>(TileShape_MNK{})));
  ...
  using SmemLayoutAtomdKV =
      std::conditional_t<!Varlen, SmemLayoutAtomdKVTMA, SmemLayoutAtomdKVSTG>;
  using SmemLayoutdKV = decltype(tile_to_shape(SmemLayoutAtomdKV{},
  ...
  using TMA_dKV = decltype(make_tma_copy(
      ...
      make_tensor(make_gmem_ptr(static_cast<Element *>(nullptr)), ShapedKV{},
                  StridedKV{}),
      SmemLayoutdKVTMA{}, select<1, 2>(TileShape_MNK{}),
      _1{})); // no mcast for dKV
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_dKV tma_store_dK =
        make_tma_copy(GmemTiledCopydKVTMA{}, mdK, SmemLayoutdKVTMA{},
                      select<1, 2>(TileShape_MNK{}), _1{}); // no mcast for dKV
    TMA_dKV tma_store_dV =
        make_tma_copy(GmemTiledCopydKVTMA{}, mdV, SmemLayoutdKVTMA{},
                      select<1, 2>(TileShape_MNK{}), _1{}); // no mcast for dKV
    return {args.ptr_dK,    args.shape_dK, args.stride_dK, args.ptr_dV,
            args.stride_dV, tma_store_dK,  tma_store_dV,   args.cu_seqlens};
  }
  ...
  store(Params const &params, FrgTensorO const &tdKrdK,
    ...
    Tensor sdK = make_tensor(
        make_smem_ptr(shared_storage.epilogue.smem_dk.data()), SmemLayoutdKV{});
    Tensor sdV = make_tensor(
        make_smem_ptr(shared_storage.epilogue.smem_dv.data()), SmemLayoutdKV{});
    ...
    if constexpr (!Varlen) {
      ...
      Tensor gdK =
          local_tile(mdK(_, _, bidh, bidb), select<1, 2>(TileShape_MNK{}),
                     make_coord(n_block, _0{})); // (M, K)
      Tensor gdV =
          local_tile(mdV(_, _, bidh, bidb), select<1, 2>(TileShape_MNK{}),
                     make_coord(n_block, _0{})); // (M, K)
      ...
      Tensor gdK = local_tile(
          ...
          select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{})); // (M, K)
      Tensor mdV =
...
#pragma unroll
      ...
      flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
          ...
          gmem_tiled_copy_dKV, tdKVrdV, tdKVgdV, tdKVcdKV, tdKVpdKV,
          seqlen - n_block * kBlockN);
      flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                  /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
          gmem_tiled_copy_dKV, tdKVrdK, tdKVgdK, tdKVcdKV, tdKVpdKV,
          seqlen - n_block * kBlockN);
    }
  }
...
#pragma unroll
    ...
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
        gmem_tiled_copy_dKV, tdKVrdKV, tdKVgdK, tdKVcdKV, tdKVpdKV,
        seqlen - n_block * kBlockN);
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
        gmem_tiled_copy_dKV, tdKVrdKV, tdKVgdV, tdKVcdKV, tdKVpdKV,
        seqlen - n_block * kBlockN);
  }
};
