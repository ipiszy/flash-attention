...
template <class TileShape_MK_, class Element, class ElementAccum, int kNThreads,
          class SmemLayoutdQaccumTMA, class TiledMma, bool dQ_swapAB>
...
public:
  ...
  static constexpr int kBlockKSmem =
      MmaShapeN % 64 == 0 ? 64 : (MmaShapeN % 32 == 0 ? 32 : 16);
  ...
  struct SharedStorage : cute::aligned_struct<128> {
    cute::array_aligned<ElementAccum, cute::cosize_v<SmemLayoutdQaccumTMA>,
                        1024>
        smem_dqacc;
  ...
  using ShapedQ = cute::Shape<int32_t, int32_t, int32_t,
  ...
  using TMA_dQaccum = decltype(make_tma_copy(
      ...
      make_tensor(make_gmem_ptr(static_cast<ElementAccum *>(nullptr)),
                  ShapedQ{}, StridedQ{}),
      SmemLayoutdQaccumTMA{}, SmemLayoutdQaccumTMA{}.shape(),
      _1{})); // no mcast for dQ
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_dQaccum tma_load_dQaccum = make_tma_copy(
        GmemTiledCopydQaccum{}, mdQaccum, SmemLayoutdQaccumTMA{},
        SmemLayoutdQaccumTMA{}.shape(), _1{}); // no mcast for dQaccum
    return {tma_load_dQaccum, args.shape_dQaccum, args.ptr_dQ,    args.shape_dQ,
            args.stride_dQ,   args.softmax_scale, args.cu_seqlens};
  }
  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    Tensor sdQ = make_tensor(make_smem_ptr(shared_storage.smem_dq.data()),
                             SmemLayoutdQ{});
    Tensor sdQt = make_tensor(make_smem_ptr(shared_storage.smem_dq.data()),
                              SmemLayoutdQt{});
    ...
    int const seqlen =
        !is_varlen ? get<0>(params.shape_dQ)
                   : params.cu_seqlens[bidb + 1] - params.cu_seqlens[bidb];
    if (is_varlen && m_block * kBlockM >= seqlen) {
      return;
    }
    ...
    int const offset_padded =
        !is_varlen ? 0
                   : ((params.cu_seqlens[bidb] + bidb * 128) / 128 * 128) *
                         (kHeadDim / get<1>(SmemLayoutdQaccumTMA{}.shape()));
    Tensor mdQaccum = params.tma_load_dQaccum.get_tma_tensor(
        params.shape_dQaccum)(_, _, bidh, !is_varlen ? bidb : 0);
    Tensor gdQaccum = local_tile(
        domain_offset(make_coord(offset_padded, _0{}), mdQaccum),
        SmemLayoutdQaccumTMA{}.shape(), make_coord(m_block, _0{})); // (M, K)
    ...
    Tensor taccdQrdQaccum =
        partition_fragment_C(tiled_mma_dQ, select < !dQ_swapAB ? 0 : 1,
...
#pragma unroll
    for (int i = 0; i < size(taccdQrdQaccum); ++i) {
      taccdQrdQaccum(i) *= params.softmax_scale;
    }
    ...
    Tensor mdQ =
        make_tensor(make_gmem_ptr(params.ptr_dQ), params.shape_dQ,
                    params.stride_dQ)(_, _, bidh, !is_varlen ? bidb : 0);
    Tensor gdQ =
        local_tile(domain_offset(make_coord(offset, _0{}), mdQ), TileShape_MK{},
...
#pragma unroll
    for (int k = 0; k < size(tdQpdQ); ++k) {
      tdQpdQ(k) = get<1>(tdQcdQ(_0{}, _0{}, k)) < get<1>(params.shape_dQ);
    }
    // Clear_OOB_K must be false since we don't want to write zeros to gmem
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
        gmem_tiled_copy_dQ, tdQrdQ, tdQgdQ, tdQcdQ, tdQpdQ,
