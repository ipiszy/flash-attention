...
public:
  ...
  static constexpr uint32_t NumMmaWarpGroups =
      CUTE_STATIC_V(size(TiledMmaSdP{})) / cutlass::NumThreadsPerWarpGroup;
  static constexpr uint32_t MaxThreadsPerBlock =
      CUTE_STATIC_V(size(TiledMmaSdP{})) +
  ...
  // uint32_t LoadRegisterRequirement = 56; static constexpr uint32_t
  ...
  struct SharedStorage {
    ...
    struct {
      ...
      alignas(16) typename CollectiveMainloop::MainloopPipeline::SharedStorage
          pipeline_q;
      alignas(16) typename CollectiveMainloop::MainloopPipeline::SharedStorage
          pipeline_do;
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    return {CollectiveMainloop::to_underlying_arguments(args.mainloop),
            CollectiveEpilogue::to_underlying_arguments(args.epilogue), hw_info,
  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    MainloopPipeline pipeline_q(shared_storage.pipeline_q, pipeline_params,
                                ClusterShape{});
    MainloopPipeline pipeline_do(shared_storage.pipeline_do, pipeline_params,
                                 ClusterShape{});
    ...
    if (warp_group_idx == 0) { // Producer
      ...
      if (warp_idx_in_warpgroup == 0) { // Load K, V, and do TMA on Q and dO
        ...
        for (auto work_tile_info =
                 scheduler.template get_initial_work</*IsProducer=*/true>(
                     params.scheduler);
          ...
          if constexpr (Varlen) {
            if (n_block * kBlockM >=
                collective_mainloop.get_seqlen_k(params.mainloop, bidb)) {
              scheduler.prefetch_next_work(params.scheduler, work_tile_info);
              continue;
            }
          }
          if constexpr (Is_causal) {
            int const m_block_min = collective_mainloop.get_m_block_min(
                params.mainloop, n_block, bidb);
            int const m_block_max = cute::ceil_div(
                collective_mainloop.get_seqlen_q(params.mainloop, bidb),
                kBlockM);
          ...
          collective_mainloop.load(params.mainloop, pipeline_q, pipeline_do,
                                   smem_pipe_write, shared_storage,
                                   scheduler_prefetch, block_coord, work_idx);
          ++work_idx;
        }
        ...
        TileScheduler scheduler(
            reinterpret_cast<typename TileScheduler::SharedStorage *>(
                &shared_storage.smem_scheduler));
        for (auto work_tile_info =
                 scheduler.template get_initial_work</*IsProducer=*/false>(
                     params.scheduler);
             work_tile_info.is_valid(params.scheduler);
             work_tile_info =
          ...
          collective_mainloop.store_dq(params.mainloop, shared_storage,
                                       block_coord);
        }
      }
    } else { // Consumer
      ...
      for (auto work_tile_info =
        ...
        if constexpr (Is_causal) {
          int const m_block_min = collective_mainloop.get_m_block_min(
              params.mainloop, n_block, bidb);
          int const m_block_max = cute::ceil_div(
              collective_mainloop.get_seqlen_q(params.mainloop, bidb), kBlockM);
          if (m_block_min >=
              m_block_max) { // We exit early and write 0 to dK and dV
            collective_epilogue.store_zero(
                params.epilogue, threadIdx.x - NumCopyThreads, block_coord);
            continue;
          }
        }
