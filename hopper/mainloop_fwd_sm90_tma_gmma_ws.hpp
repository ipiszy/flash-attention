...
struct CollectiveMainloopFwdSm90 {
    ...
    using KVBlockScaleCopyAtom = Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS_ZFILL<float>, float>;
    using KVTokenScaleCopyAtom = Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS_ZFILL<uint128_t>, float>;
    ...
    static constexpr int kScaleElemsPerLoad = sizeof(cute::uint128_t) / sizeof(float);
    static_assert(
        shape<1>(TileShape_MNK{}) % kScaleElemsPerLoad == 0, 
        "TileN must be a multiples of kScaleElemsPerLoad."
    );
    static constexpr int kScaleThreadsPerBlock = shape<1>(TileShape_MNK{}) / kScaleElemsPerLoad;
    static constexpr int kScaleThreads = (
        kScaleThreadsPerBlock <= NumProducerThreads? 
        kScaleThreadsPerBlock : 
        (kScaleThreadsPerBlock / 2 <= NumProducerThreads ? kScaleThreadsPerBlock / 2 : kScaleThreadsPerBlock / 4)
    );
    static_assert(kScaleThreadsPerBlock % kScaleThreads == 0);
    static_assert(kScaleThreads <= NumProducerThreads);
    using GmemLayoutKVPerTokenScaleCpAsyncAtom = 
        Layout<Shape<_1, Int<kScaleThreads>>, Stride<Int<kScaleThreads>, _1>>;
    using GmemTiledCopyKVPerTokenScaleCpAsync = decltype(
        make_tiled_copy(KVTokenScaleCopyAtom{},
                        GmemLayoutKVPerTokenScaleCpAsyncAtom{},
                        Layout<Shape<_1, Int<kScaleElemsPerLoad>>>{}));  // Val layout, 4 floats per load
    using GmemTiledCopyKVPerBlockScaleCpAsync = decltype(
        make_tiled_copy(KVBlockScaleCopyAtom{},
                        Layout<Shape<_1, _1>>{},
                        Layout<Shape<_1, _1>>{}));  // Val layout, 1 floats per load
    using SmemLayoutKPerTokenScale = Layout<
        Shape<Int<kStages>, decltype(cute::get<1>(TileShape_MNK{}))>, 
        Stride<decltype(cute::get<1>(TileShape_MNK{})), _1>>;
    using SmemLayoutKPerBlockScale = Layout<Shape<Int<kStages>, _1>, Stride<_1, _1>>;
    ...
    using PrefetchVScaleShape = cute::Shape<_3>;
    ...
    // using TMA_Q_PER_TOKEN_SCALE = decltype(make_tma_copy(
    //     GmemTiledCopyQ{},
    //     make_tensor(make_gmem_ptr(static_cast<float const*>(nullptr)), ShapeDescale{}, StrideDescale{}),
    ...
    // static constexpr uint32_t TmaTransactionBytesQPerTokenScale = static_cast<uint32_t>(size(SmemLayoutQPerTokenScale{}) * cutlass::sizeof_bits_v<float> / 8);
    ...
    using SmemKVScaleLayoutType = std::conditional_t<
        Is_FP8 && kScalingRecipe == ScalingRecipe::PerQKVToken, 
        SmemLayoutKPerTokenScale,
        std::conditional_t<
            Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock, 
            SmemLayoutKPerBlockScale,
            nullptr_t
        >
    >;
    using SmemKVScaleType = std::conditional_t<
        Is_FP8 && kScalingRecipe == ScalingRecipe::PerQKVToken, 
        cute::array_aligned<float, cute::cosize_v<SmemLayoutKPerTokenScale>>,
        std::conditional_t<
            Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock, 
            cute::array_aligned<float, cute::cosize_v<SmemLayoutKPerBlockScale>>,
            nullptr_t
        >
    >;
    struct TensorStorageWithoutPNoTranspose : cute::aligned_struct<cute::max(SmemAlignmentQ, SmemAlignmentK, SmemAlignmentVtNoTranspose), _0> {
        ...
        SmemKVScaleType smem_k_scale;
        SmemKVScaleType smem_v_scale;
        SmemQv_t smem_qv;
    };
    ...
    static constexpr bool UseSchedulerBarrier = (IntraWGOverlap
        ...
        : NumMmaWarpGroups == 2)
        && !LargeHeadDimV;
    static constexpr bool RescaleOBeforeGemm = (kHeadDim > 128 && (!Is_FP8 || V_colmajor)) || (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock);
    ...
    struct Params {
        ...
        TMA_Q tma_load_Q;
        // TMA_Q_PER_TOKEN_SCALE tma_load_Q_per_token_scale;
        TMA_K tma_load_K;
        TMA_V tma_load_V;
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        // TMA_Q_PER_TOKEN_SCALE tma_load_Q_per_token_scale = make_tma_copy(
        //     GmemTiledCopyQ{},
        //     mQ_per_token_scale,
        //     SmemLayoutQPerTokenScale{},
        //     ClusterShape{}); // no mcast for Q
        ...
        // printf("\n");
        // printf("tma_load_Q_per_token_scale: \n");
        // print(tma_load_Q_per_token_scale);
        // printf("\n");
    ...
    load(Params const& params,
        ...
        auto [tQgQ_per_token_scale, tQsQ_per_token_scale] = [&] {
            if constexpr (Is_FP8 && kScalingRecipe != ScalingRecipe::PerKVHead) {
                ...
                // Tensor tQgQ_per_token_scale = group_modes<0, 2>(thr_scale_copy_q.partition_S(gQ_per_token_scale));
                ...
                // {
                //     CUTE_LOG("mQ_per_token_scale: (0): %f, (1): %f\n", mQ_per_token_scale(0), mQ_per_token_scale(1));
                //     CUTE_LOG("gQ_per_token_scale: (0): %f, (1): %f\n", gQ_per_token_scale(0), gQ_per_token_scale(1));
                // }
        ...
        auto scale_copy_kv = [&] {
            if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                return GmemTiledCopyKVPerBlockScaleCpAsync{};
            } else if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQKVToken) {
                return GmemTiledCopyKVPerTokenScaleCpAsync{};
            } else {
                return nullptr;
            }
        }();
        auto [tKgK_scale, tKsK_scale, tVgV_scale, tVsV_scale] = [&] {
            if constexpr (Is_FP8 && (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock || kScalingRecipe == ScalingRecipe::PerQKVToken)) {
                ThrCopy thr_scale_copy_kv = scale_copy_kv.get_slice(threadIdx.x);
                const static int kMinContiguousElems = (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) ? kBlockN : sizeof(uint128_t) / sizeof(float);
                const static int kMultiplier = (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) ? 1 : kMinContiguousElems;
                Tensor mK_scale = make_tensor(make_gmem_ptr(params.ptr_k_descale), make_layout(_1{}, params.shape_k_descale, params.stride_k_descale))(_, _, bidh_kv);
                Tensor mV_scale = make_tensor(make_gmem_ptr(params.ptr_v_descale), make_layout(_1{}, params.shape_v_descale, params.stride_v_descale))(_, _, bidh_kv);
                Tensor gK_scale = domain_offset(
                    make_coord(_0{}, (seqlen_info.seqlen_k_static + kMinContiguousElems - 1) / kMinContiguousElems * kMultiplier * bidb_kv), 
                    mK_scale); 
                Tensor gV_scale = domain_offset(
                    make_coord(_0{}, (seqlen_info.seqlen_k_static + kMinContiguousElems - 1) / kMinContiguousElems * kMultiplier * bidb_kv), 
                    mV_scale);
                Tensor tKgK_scale = thr_scale_copy_kv.partition_S(gK_scale);
                Tensor tVgV_scale = thr_scale_copy_kv.partition_S(gV_scale);
                Tensor sK_scale = make_tensor(make_smem_ptr(shared_storage.tensors.mainloop.smem_k_scale.data()), SmemKVScaleLayoutType{});
                Tensor sV_scale = make_tensor(make_smem_ptr(shared_storage.tensors.mainloop.smem_v_scale.data()), SmemKVScaleLayoutType{});
                Tensor tKsK_scale = thr_scale_copy_kv.partition_D(sK_scale);
                Tensor tVsV_scale = thr_scale_copy_kv.partition_D(sV_scale);

                if (threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                    printf("mK_scale: \n");
                    print(mK_scale);
                    print_tensor(mK_scale);
                    printf("\n");
                    printf("gK_scale: \n");
                    print(gK_scale);
                    print_tensor(gK_scale);
                    printf("\n");
                    printf("tKgK_scale: \n");
                    print(tKgK_scale);
                    printf("\n");
                ...
                return cute::make_tuple(tKgK_scale, tKsK_scale, tVgV_scale, tVsV_scale);
            } else {
                return cute::make_tuple(nullptr, nullptr, nullptr, nullptr);
            }
        }();
            ...
            if constexpr (!PagedKV) {
                if constexpr (Is_FP8 && (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock || kScalingRecipe == ScalingRecipe::PerQKVToken)) {
                    if (should_load_scales) {
                        copy(scale_copy_kv, tKgK_scale(_, _, n_block), tKsK_scale(_, smem_pipe_write.index(), _));
                        // copy(scale_copy_v_per_block, tVgV_scale(_, n_block), tVsV_scale(_, smem_pipe_write.index()));
        ...
        auto copy_Vt_to_V = [&] (int const n_block, auto const& smem_pipe_write, bool should_load_scales) {
            ...
            if constexpr (Is_FP8 && (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock || kScalingRecipe == ScalingRecipe::PerQKVToken)) {
                if (should_load_scales) {
                    ...
                    // if (blockIdx.x == 8 && blockIdx.y == 0 && blockIdx.z == 0) {
                    //     CUTE_LOG(
                    //         "producer load v_descale, n_block: %d, smem_pipe_write.index(): %d, sV_per_block_scale(1): %f, sV_per_block_scale(0): %f, src: %f, dst: %f\n",
                    //         n_block, smem_pipe_write.index(), sV_per_block_scale(1), sV_per_block_scale(0), tVgV_scale(_0{}, n_block), tVsV_scale(_0{}, smem_pipe_write.index())
                    //     );
                    // }
                    copy(scale_copy_kv, tVgV_scale(_, _, n_block), tVsV_scale(_, smem_pipe_write.index(), _));
                }
            }                
            ...
            if constexpr (Is_FP8 && (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock || kScalingRecipe == ScalingRecipe::PerQKVToken)) {
                if (should_load_scales) {
                    ...
                    // if (blockIdx.x == 8 && blockIdx.y == 0 && blockIdx.z == 0) {
                    // CUTE_LOG("clock: %lld, after producer commit, n_block: %d, smem_pipe_write: %d, phase: %d\n", clock64(), n_block, smem_pipe_write.index(), smem_pipe_write.phase());
                    // }
                } else {
        ...
        bool should_load_scales = (!Use_TMA_KV && thread_idx < kScaleThreads) || ((SingleProducerWarp || warp_idx_in_warpgroup == 0) && elected);
        ...
        if constexpr (Is_FP8 && kScalingRecipe != ScalingRecipe::PerKVHead) {
            if (params.ptr_q_descale) {
                // CUTE_LOG("%s\n", "Before loading Q per token scale.");
                ...
                // CUTE_LOG("%s\n", "After loading Q per token scale.");
            }
        }
    ...
    load_tail(MainloopPipelineK pipeline_k, MainloopPipelineV pipeline_v, MainloopPipelineVt pipeline_vt,
              MainloopPipelineKVScaling pipeline_k_scaling, MainloopPipelineKVScaling pipeline_v_scaling, 
    ...
    mma(Params const& params,
        ...
        auto scoremod_premask_fn = [&](auto& tSrS, auto& tQscale_row, auto& tKscale_col, int stage, int m_block, int n_block) {
            ...
            if constexpr (Is_FP8) {
                ...
                if constexpr (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                    ...
                    // if ((threadIdx.x == 128 || threadIdx.x == 256) && bidb == 8 && bidh == 0 && m_block == 1) {
                    // CUTE_LOG("inside s_fn: k_descale(%d): %f, prev: %f\n", n_block, tKscale_col(1), tKscale_col(0));
                    // }
                    ...
                    for (int m = 0; m < size<Row>(tSrS_rowcol); ++m) {
                        ...
                        tQscale_row(m) *= ratio;
                        // if ((threadIdx.x == 128 || threadIdx.x == 256) && bidb == 8 && bidh == 0 && m_block == 1) {
                        // CUTE_LOG("inside s_fn: n_block: %d, ratio: %f, q_descale x k_ratio (%d): %f\n", n_block, ratio, m, tQscale_row(m));
                        // }
                    }
               }
            }
        ...
        if constexpr (!AppendKV) {
            ...
            if constexpr (Is_FP8 && kScalingRecipe != ScalingRecipe::PerKVHead) {
                if (params.ptr_q_descale) {
                    // CUTE_LOG("Before waiting for Q per token scale, work_idx: %d\n", work_idx);
                    barrier_Q_scale.wait(work_idx % 2);
                    // CUTE_LOG("After waiting for Q per token scale, work_idx: %d\n", work_idx);
                }
            }
        } else {
        ...
        if constexpr (IntraWGOverlap) {
            ...
            auto tKscale_col = [&] {
                if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                    ...
                    if constexpr (kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                        ...
                        // if ((threadIdx.x == 128 || threadIdx.x == 256) && bidb == 8 && bidh == 0 && m_block == 1) {
                        //     CUTE_LOG("loaded: k_descale(%d): %f, prev: %f\n", n_block, tKscale_col(1), tKscale_col(0));
                        // }
                    }
                    return tKscale_col;
                } else {
            ...
            auto tVscale_row = [&]() {
                Tensor tVscale_row = make_tensor<float>(PrefetchVScaleShape{});
                tVscale_row(0) = 1.0f;
                if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
            ...
            auto fwd_step = [&](auto& tQscale_row, auto& tKscale_col, auto& tVscale_row, int const n_block, auto mask_fn, auto check_inf_type) {
                ...
                if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                    ...
                    tKscale_col(1) = sK_per_scale(smem_pipe_read.index());
                    // tVscale_row(2) = sV_per_scale(smem_pipe_read.index());
                    // if ((threadIdx.x == 128 || threadIdx.x == 256) && bidb == 8 && bidh == 0 && m_block == 1) {
                    //     CUTE_LOG("loaded: k_descale(%d): %f, prev: %f\n", n_block - 1, tKscale_col(1), tKscale_col(0));
                    // }
                }

                if constexpr(!HasQv) {
                    if (!UseSchedulerBarrier || warp_group_idx == 0) { 
                        // if (blockIdx.x == 256 && bidb == 8 && bidh == 0 && m_block == 1) {
                        // CUTE_LOG("clock64: %lld, before consumer wait, n_block: %d, smem_pipe_read_v.idx: %d, smem_pipe_read_v.phase: %d\n", clock64(), n_block, smem_pipe_read_v.index(), smem_pipe_read_v.phase());
                        // }
                        consumer_wait(pipeline_v, smem_pipe_read_v); }
                        // if (blockIdx.x == 256 && bidb == 8 && bidh == 0 && m_block == 1) {
                        // CUTE_LOG("clock64: %lld, after consumer wait, n_block: %d, smem_pipe_read_v.idx: %d, smem_pipe_read_v.phase: %d\n", clock64(), n_block, smem_pipe_read_v.index(), smem_pipe_read_v.phase());
                        // }
                }
                if constexpr (RescaleOBeforeGemm) { 
                    if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                        // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && threadIdx.x == 128) {
                        // CUTE_LOG("n_block: %d, smem_pipe_read_v.index(): %d, v_descale: %f, v_descale_prev: %f\n", n_block, smem_pipe_read_v.index(), v_descale, v_descale_prev);
                        // }
                        tVscale_row(0) = tVscale_row(1);
                        tVscale_row(1) = sV_per_scale(smem_pipe_read_v.index());
                        // if ((threadIdx.x == 128 || threadIdx.x == 256) && bidb == 8 && bidh == 0 && m_block == 1) {
                        //     CUTE_LOG(
                        //         "loaded: v_descale(n_block=%d, smem_pipe_read_v.index()=%d, phase() = %d): %f, prev: %f\n", 
                        //         n_block, smem_pipe_read_v.index(), smem_pipe_read_v.phase(), tVscale_row(1), tVscale_row(0)
                        //     );
                        // }
                ...
                if constexpr (!HasQv) {
                    warpgroup_wait<0>();
                    pipeline_v.consumer_release(smem_pipe_read_v);  // release V
                    // if (bidb == 8 && bidh == 0 && m_block == 1 && (threadIdx.x == 128 || threadIdx.x == 256)) {
                    // CUTE_LOG("clock64: %lld, after consumer release, n_block: %d, smem_pipe_read_v.idx: %d, smem_pipe_read_v.phase: %d\n", clock64(), n_block, smem_pipe_read_v.index(), smem_pipe_read_v.phase());
                    // }
                }
                if constexpr (Is_FP8 && !V_colmajor) { flash::permute_Cregs_fp8(tSrS); }
            ...
            // Tell producers that smem_q is ready
            cutlass::arch::NamedBarrier::arrive(NumMmaThreadsQK + (Use_TMA_Q ? cutlass::NumThreadsPerWarp : NumProducerThreads), static_cast<uint32_t>(FwdNamedBarriers::QueryEmpty) /*id*/);
            // CUTE_LOG("%s\n", "after consumer arrive");
            if constexpr (!HasQv) { 
                consumer_wait(pipeline_v, smem_pipe_read); 
                // if (bidb == 8 && bidh == 0 && m_block == 1 && (threadIdx.x == 128 || threadIdx.x == 256)) {
                // CUTE_LOG("clock64: %lld, after consumer wait, n_block: %d, smem_pipe_read.idx: %d, smem_pipe_read.phase: %d\n", clock64(), n_block, smem_pipe_read.index(), smem_pipe_read.phase());
                // }
            }
            if constexpr (RescaleOBeforeGemm) { 
                if constexpr (Is_FP8 && kScalingRecipe == ScalingRecipe::PerQTokenKVBlock) {
                    // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && threadIdx.x == 128) {
                    // CUTE_LOG("n_block: %d, v_descale: %f, v_descale_prev: %f\n", n_block, v_descale, v_descale_prev);
                    // }
                    tVscale_row(0) = tVscale_row(1);
                    tVscale_row(1) = sV_per_scale(smem_pipe_read.index());
                    // if ((threadIdx.x == 128 || threadIdx.x == 256) && bidb == 8 && bidh == 0 && m_block == 1) {
                    //     CUTE_LOG("loaded: v_descale(%d): %f, prev: %f\n", n_block, tVscale_row(1), tVscale_row(0));
                    // }
                    softmax.rescale_o</*use_additional_scale=*/true>(tOrO, scores_scale, tVscale_row(0) / tVscale_row(1)); 
                    // tVscale_row(0) = tVscale_row(1);
                    // tVscale_row(1) = tVscale_row(2);
                } else {
                    softmax.rescale_o(tOrO, scores_scale); 
                }
            }
            ...
            // if (bidb == 8 && bidh == 0 && m_block == 1 && (threadIdx.x == 128 || threadIdx.x == 256)) {
            // CUTE_LOG("clock64: %lld, after consumer release, n_block: %d, smem_pipe_read.idx: %d, smem_pipe_read.phase: %d\n", clock64(), n_block, smem_pipe_read.index(), smem_pipe_read.phase());
            // }
            softmax.rescale_o(tOrO, scores_scale); 
