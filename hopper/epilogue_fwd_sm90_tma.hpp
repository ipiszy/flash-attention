...
struct CollectiveEpilogueFwd {
  ...
  using SharedStorage =
      cute::array_aligned<Element, cute::cosize_v<SmemLayoutO>>;
  ...
  using TiledCopyOAtom =
      cute::Copy_Atom<cute::UniversalCopy<cutlass::uint128_t>, Element>;
  ...
  using TiledCopyO = decltype(make_tiled_copy(
      TiledCopyOAtom{}, TiledCopyOThrLayout{}, // Thr layout
  ...
  static void prefetch_tma_descriptors(Params const &epilogue_params) {
    if constexpr (!Seqlen_traits::kUseVarSeqLen) {
      cute::prefetch_tma_descriptor(
          epilogue_params.tma_store_O.get_tma_descriptor());
    }
  }
  ...
  template <typename SharedStorage, typename FrgTensorO, typename FrgTensorLSE,
            typename TiledMma>
  CUTLASS_DEVICE void
  store(Params const &epilogue_params, FrgTensorO const &tOrO,
    ...
    cutlass::arch::NamedBarrier::sync(
        NumMmaThreads, static_cast<int>(FwdNamedBarriers::ValueEmpty) /*id*/);
    ...
    // taccOcO has shape ((2, 2, V), MMA_M, MMA_K), we only take only the row
    // indices.
...
#pragma unroll
  ...
  template <typename SharedStorage, typename FrgTensorO, typename FrgTensorLSE,
            typename TiledMma>
  CUTLASS_DEVICE void
  store_fp8(Params const &epilogue_params, FrgTensorO const &tOrO,
            FrgTensorLSE const &lse, SharedStorage &shared_storage,
            TiledMma tiled_mma, int thread_idx,
            cute::tuple<int32_t, int32_t, int32_t> const &block_coord,
            const Seqlen_traits &seqlen_traits_q) {
    ...
    Tensor sOacc = make_tensor(make_smem_ptr(shared_storage.smem_o.data()),
                               SmemLayoutrO{});
    auto rmem_thr_copy_O = rmem_tiled_copy_O.get_thread_slice(thread_idx);

    Tensor taccOsO = rmem_thr_copy_O.partition_D(sOacc);
    Tensor tOrO_out =
        flash::convert_type<Element>(tOrO); // Element is Ktraits::OutputType
    ...
    int const seqlen_q = [&] {
      if constexpr (Seqlen_traits::kUseVarSeqLen) {
        return seqlen_traits_q.actual_seq_len;
      } else {
        return shape<2>(epilogue_params.layout_LSE);
      }
    }();
    if (get<1>(taccOcO_row(_0{})) == 0) {
#pragma unroll
      for (int mi = 0; mi < size(lse); ++mi) {
        const int row = get<0>(taccOcO_row(mi));
        if (row < seqlen_q - m_block * kBlockM) {
          gLSE(row) = lse(mi);
        }
      }
    }
  ...
  CUTLASS_DEVICE void
  store_zero(Params const &epilogue_params, SharedStorage &shared_storage,
             int thread_idx,
             cute::tuple<int32_t, int32_t, int32_t> const &block_coord,
             const Seqlen_traits &seqlen_traits_q) {
    auto [m_block, bidh, bidb] = block_coord;
...
#pragma unroll
    ...
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
        ...
        get<0>(epilogue_params.layout_O.shape()) - m_block * kBlockM);
    static_assert(kBlockM <= NumMmaThreads);
    if (thread_idx <
        get<0>(epilogue_params.layout_LSE.shape()) - m_block * kBlockM) {
