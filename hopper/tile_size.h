...
constexpr std::tuple<int, int, bool, bool> tile_size_fwd_sm90(
    ...
    if (element_size == 2) {
        if (headdim <= 64) {
            ...
            // return {64, is_causal || is_local || paged_kv ? 128 : 176, true, true};
            return {64, is_causal || is_local || paged_kv ? 32 : 32, true, true};
            // return {128, is_causal || is_local || paged_kv ? 32 : 32, true, true};
            ...
            return {64, paged_kv || is_local ? 32 : 32, false, true};  // 128 x 112 hits the limit of smem
        } else {
