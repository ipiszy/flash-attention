...
struct CollectiveMainloopBwd {
  ...
  static_assert(!(SdP_swapAB &&
                  dKV_swapAB)); // If SdP_swapAB, then we don't swap for dKV
  ...
  static constexpr bool Mma_dQ_is_RS =
      AtomLayoutMSdP == 2 && AtomLayoutMdQ == 2 && !SdP_swapAB &&
  ...
  using TileShapeAtomdQ = std::conditional_t<
      !dQ_swapAB,
      Shape<Int<kBlockM>, Int<kHeadDim / (NumdQWarpGroups / AtomLayoutMdQ)>,
            Int<kBlockN>>,
      ...
      std::conditional_t<
          ...
          std::conditional_t<
              ...
              decltype(cute::GMMA::rs_op_selector<
                       Element, Element, ElementAccum, TileShapeAtomdQ,
                       GMMA::Major::K, GMMA::Major::MN>()),
              decltype(cute::GMMA::ss_op_selector<
                       Element, Element, ElementAccum, TileShapeAtomdQ,
                       GMMA::Major::K, GMMA::Major::MN>())>,
          decltype(cute::GMMA::ss_op_selector<Element, Element, ElementAccum,
                                              TileShapeAtomdQ, GMMA::Major::MN,
                                              GMMA::Major::K>())>{},
      AtomLayoutdQ{}));
  ...
  using SmemLayoutAtomQ =
      decltype(cutlass::gemm::collective::detail::ss_smem_selector<
               ...
               Int<dKV_swapAB ? kHeadDim : kHeadDim / (2 / AtomLayoutNdKV)>>());
  ...
  using SmemLayoutAtomK =
      decltype(cutlass::gemm::collective::detail::ss_smem_selector<
               GMMA::Major::K, Element, Int<kBlockN>,
               Int<dQ_swapAB
                       ? kHeadDim
                       : kHeadDim / (NumdQWarpGroups / AtomLayoutMdQ)>>());
  using SmemLayoutK =
  ...
  using SmemLayoutdS = decltype(tile_to_shape(
      SmemLayoutAtomdS{},
      make_shape(Int<kBlockM>{}, Int<kBlockN>{}, Int<kStages>{})));

  // Need stride to be multiple of 32, otherwise we get error (misaligned
  // address) when doing TMA if e.g. kBlockM=80
  using SmemLayoutLSE =
      cute::Layout<cute::Shape<Int<kBlockM>, Int<kStages>>,
                   cute::Stride<_1, Int<cute::round_up(kBlockM, 32)>>>;
  using SmemLayoutLSEMma =
      cute::Layout<cute::Shape<Int<kBlockN>, Int<kBlockM>, Int<kStages>>,
                   cute::Stride<_0, _1, Int<cute::round_up(kBlockM, 32)>>>;
  ...
  using R2STiledCopydQaccum = decltype(make_tiled_copy(
      Copy_Atom<DefaultCopy, ElementAccum>{}, R2SLayoutAtomdQaccum{},
      Layout<Shape<_4>>{})); // Val layout, 4 vals per store
  using SmemLayoutdQaccum = Layout<Shape<Int<kBlockM * kHeadDim>>, Stride<_1>>;
  // We want dQaccum smem to have last dimension 32, so that we only need to do
  // 1 TMA instruction. The layout Layout_K_SW128_Atom<ElementAccum> has 32
  // elements per row.
  // // TMA limit is that each dimension in smem must be <= 256.
  // static constexpr int ElemsPerRowTMA = (kBlockM * kHeadDim) / 32 <= 256 ? 32
  // : 64;
  static constexpr int ElemsPerRowTMA =
      32; // If we change this, we'll also need to change the dQ shape in host.
  static_assert((kBlockM * kHeadDim) % ElemsPerRowTMA == 0);
  using TileShape_dQaccum =
      cute::Shape<Int<(kBlockM * kHeadDim) / ElemsPerRowTMA>,
                  Int<ElemsPerRowTMA>>;
  // using TileShape_dQaccum = cute::Shape<Int<kBlockM>, Int<kHeadDim>>;
  using SmemLayoutdQaccumTMA = decltype(tile_to_shape(
      GMMA::Layout_K_SW128_Atom<ElementAccum>{}, TileShape_dQaccum{}));
  using SmemLayoutdQaccumTMANoSwizzle =
      decltype(get_nonswizzle_portion(SmemLayoutdQaccumTMA{}));

  using SmemCopyAtomPdS =
      Copy_Atom<std::conditional_t<!SdP_swapAB, cute::SM90_U32x4_STSM_N,
                                   cute::SM90_U16x8_STSM_T>,
                Element>;
  using SmemCopyAtomdKV =
      Copy_Atom<std::conditional_t<!dKV_swapAB, cute::SM90_U32x4_STSM_N,
                                   cute::SM90_U16x8_STSM_T>,
                Element>;
  ...
  using ShapeQKV = cute::Shape<int32_t, int32_t, int32_t,
                               int32_t>; // (seqlen, d, head, batch)
  ...
  using TMA_K = decltype(make_tma_copy(
      ...
      make_tensor(make_gmem_ptr(static_cast<Element const *>(nullptr)),
                  ShapeQKV{}, StrideQKV{}),
      SmemLayoutK{}, select<1, 2>(TileShape_MNK{}), _1{})); // no mcast for KV
  ...
  using TMA_add_dQ = decltype(make_tma_copy(
      ...
      make_tensor(make_gmem_ptr(static_cast<ElementAccum *>(nullptr)),
                  ShapeQKV{}, StrideQKV{}),
      SmemLayoutdQaccumTMA{}, TileShape_dQaccum{}, _1{})); // no mcast for dQ

  using TMA_LSE = decltype(make_tma_copy(
      GmemTiledCopyLSE{},
      make_tensor(make_gmem_ptr(static_cast<ElementAccum const *>(nullptr)),
                  ShapeLSE{}, StrideLSE{}),
      select<0>(SmemLayoutLSE{}), select<0>(TileShape_MNK{}),
      _1{})); // no mcast for LSE
  ...
  static constexpr uint32_t TmaTransactionBytesLSE =
      static_cast<uint32_t>(size(select<0>(SmemLayoutLSE{})) *
                            cutlass::sizeof_bits_v<ElementAccum> / 8);

  struct TensorStorage : cute::aligned_struct<1024> {
    cute::array_aligned<Element, cute::cosize_v<SmemLayoutK>> smem_k;
    cute::array_aligned<Element, cute::cosize_v<SmemLayoutV>> smem_v;
    // It's important that smem_dqacc is aligned to 1024 bytes for the TMA, so
    // that the 1st row has no swizzle. If the address is only 128 bytes
    // aligned, it's possible that the 1st row has swizzle and when we read it
    // back in the postprocess kernel, the swizzle will not match.
    cute::array_aligned<ElementAccum, cute::cosize_v<SmemLayoutdQaccum>, 1024>
        smem_dqacc;
  ...
  static constexpr int SharedStorageQdOSize =
      sizeof(decltype((TensorStorage{}).smem_q)) +
      sizeof(decltype((TensorStorage{}).smem_do)) +
      sizeof(decltype((TensorStorage{}).smem_ds)) +
      sizeof(decltype((TensorStorage{}).smem_dqacc));
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_K tma_load_K =
        make_tma_copy(GmemTiledCopyKV{}, mK, SmemLayoutK{},
    ...
    TMA_add_dQ tma_add_dQ =
        make_tma_copy(GmemTiledCopydQaccum{}, mdQaccum, SmemLayoutdQaccumTMA{},
                      TileShape_dQaccum{}, _1{}); // no mcast for dQaccum
    Tensor mLSE = make_tensor(make_gmem_ptr(args.ptr_LSE_log2), args.shape_LSE,
                              args.stride_LSE_log2);
    TMA_LSE tma_load_LSE =
        make_tma_copy(GmemTiledCopyLSE{}, mLSE, select<0>(SmemLayoutLSE{}),
                      select<0>(TileShape_MNK{}), _1{}); // no mcast for LSE
    Tensor mdPsum = make_tensor(make_gmem_ptr(args.ptr_dPsum), args.shape_LSE,
                                args.stride_dPsum);
    TMA_LSE tma_load_dPsum =
        make_tma_copy(GmemTiledCopyLSE{}, mdPsum, select<0>(SmemLayoutLSE{}),
                      select<0>(TileShape_MNK{}), _1{}); // no mcast for dPsum
    ...
    return {args.shape_Q,
            ...
            args.shape_dQaccum,
            cutlass::FastDivmod(
                cute::ceil_div(get<2>(args.shape_Q), get<2>(args.shape_K))),
            tma_load_Q,
            tma_load_dO,
            tma_load_K,
            tma_load_V,
  ...
  CUTLASS_DEVICE void
  load(Params const &params, MainloopPipeline pipeline_q,
       MainloopPipeline pipeline_do, PipelineState &smem_pipe_write,
       SharedStorage &shared_storage,
       SchedulerPrefetch const &scheduler_prefetch,
       cute::tuple<int32_t, int32_t, int32_t> block_coord, int work_idx) {

    Tensor sQ = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_q.data()), SmemLayoutQ{});
    Tensor sdO = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_do.data()), SmemLayoutdO{});
    Tensor sK = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_k.data()), SmemLayoutK{});
    Tensor sV = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_v.data()), SmemLayoutV{});
    Tensor sLSE =
        make_tensor(make_smem_ptr(shared_storage.mainloop.smem_lse.data()),
                    SmemLayoutLSE{});
    Tensor sdPsum =
        make_tensor(make_smem_ptr(shared_storage.mainloop.smem_dpsum.data()),
                    SmemLayoutLSE{});
    ...
    Tensor mV = params.tma_load_V.get_tma_tensor(params.shape_K)(
        _, _, bidh_kv, !is_varlen_k ? bidb : 0);
    Tensor mLSE = params.tma_load_LSE.get_tma_tensor(params.shape_LSE)(
        _, bidh, !is_varlen_q ? bidb : 0);
    Tensor mdPsum = params.tma_load_dPsum.get_tma_tensor(params.shape_LSE)(
        _, bidh, !is_varlen_q ? bidb : 0);
    ...
    int const offset_padded =
        !is_varlen_q ? 0 : (params.cu_seqlens_q[bidb] + bidb * 128) / 128 * 128;
    ...
    Tensor gLSE =
        local_tile(domain_offset(make_coord(offset_padded), mLSE),
                   select<0>(TileShape_MNK{}), make_coord(_)); // (M, _)
    Tensor gdPsum =
        local_tile(domain_offset(make_coord(offset_padded), mdPsum),
                   select<0>(TileShape_MNK{}), make_coord(_)); // (M, _)
    ...
    auto [tQgQ, tQsQ] = tma_partition(
        params.tma_load_Q, block_rank_in_cluster, Layout<ClusterShape>{},
        group_modes<0, 2>(sQ), group_modes<0, 2>(gQ)); // (TMA, k), (TMA, PIPE)
    auto [tdOgdO, tdOsdO] =
        tma_partition(params.tma_load_dO, block_rank_in_cluster,
                      Layout<ClusterShape>{}, group_modes<0, 2>(sdO),
                      group_modes<0, 2>(gdO)); // (TMA, k), (TMA, PIPE)
    ...
    auto [tLSEgLSE, tLSEsLSE] =
        tma_partition(params.tma_load_LSE, _0{}, Layout<_1>{}, sLSE,
                      gLSE); // (TMA, k), (TMA, PIPE)
    auto [tLSEgdPsum, tLSEsdPsum] =
        tma_partition(params.tma_load_dPsum, _0{}, Layout<_1>{}, sdPsum,
                      gdPsum); // (TMA, k), (TMA, PIPE)
    ...
    if constexpr (cute::is_same_v<GmemTiledCopyQdO, SM90_TMA_LOAD_MULTICAST>) {
      ...
      for (int n = 0; n < size<1>(block_layout); ++n) {
        mcast_mask_qdo |=
            (uint16_t(1) << block_layout(n, cluster_local_block_id.x, _0{}));
      }
    }

    int m_block_max =
        cute::ceil_div(get_seqlen_q(params, bidb), get<0>(TileShape_MNK{}));
    ...
    // static_cast<int>(BwdNamedBarriers::QueryEmpty) /*id*/);

    if (lane_predicate) {
      // Copy K tile and V tile from GMEM to SMEM.
      shared_storage.barrier_KV.arrive_and_expect_tx(TmaTransactionBytesK +
      ...
      copy(params.tma_load_LSE.with(
               *pipeline_q.producer_get_barrier(smem_pipe_write), 0),
...
#pragma unroll 2
      for (; m_block < m_block_max - 1; ++m_block) {
        ...
        copy(params.tma_load_dO.with(
                 *pipeline_do.producer_get_barrier(smem_pipe_write),
                 mcast_mask_qdo),
             tdOgdO(_, m_block), tdOsdO(_, smem_pipe_write.index()));
        copy(params.tma_load_dPsum.with(
                 *pipeline_do.producer_get_barrier(smem_pipe_write), 0),
             tLSEgdPsum(_, m_block), tLSEsdPsum(_, smem_pipe_write.index()));
        ++smem_pipe_write;
        pipeline_q.producer_acquire(smem_pipe_write);
        copy(params.tma_load_Q.with(
                 *pipeline_q.producer_get_barrier(smem_pipe_write),
  ...
  CUTLASS_DEVICE void
  store_dq(Params const &params, SharedStorage &shared_storage,
           cute::tuple<int32_t, int32_t, int32_t> block_coord) {

    Tensor sdQ =
        make_tensor(make_smem_ptr(shared_storage.mainloop.smem_dqacc.data()),
    ...
    bool const is_varlen_q = Varlen && params.cu_seqlens_q != nullptr;
    // We reshaped dQaccum to have last dimension 32, so the offset needs to be
    // multiplied by kHeadDim / 32
    int const offset_padded =
        !is_varlen_q ? 0
                     : ((params.cu_seqlens_q[bidb] + bidb * 128) / 128 * 128) *
                           (kHeadDim / ElemsPerRowTMA);
    // Prepare the TMA loads
    Tensor mdQaccum = params.tma_add_dQ.get_tma_tensor(params.shape_dQaccum)(
        _, _, bidh, !is_varlen_q ? bidb : 0);
    Tensor gdQaccum =
        local_tile(domain_offset(make_coord(offset_padded, _0{}), mdQaccum),
                   TileShape_dQaccum{}, make_coord(_, _0{})); // (M, K, _)
...
#pragma unroll 2
    for (; m_block < m_block_max; ++m_block) {
      ...
      tma_store_wait<0>();
      if constexpr (Deterministic) {
        Barrier::arrive_inc(lock_ptr, threadIdx.x % cutlass::NumThreadsPerWarp,
  ...
  CUTLASS_DEVICE void mma(Params const &params, MainloopPipeline pipeline_q,
    ...
    Tensor sQt = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_q.data()), SmemLayoutQt{});
    Tensor sdOt = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_do.data()), SmemLayoutdOt{});
    Tensor sKt = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_k.data()), SmemLayoutKt{});
    Tensor sdS = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_ds.data()), SmemLayoutdS{});
    Tensor sdSt = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_ds.data()), SmemLayoutdSt{});
    Tensor sdQ =
        make_tensor(make_smem_ptr(shared_storage.mainloop.smem_dqacc.data()),
                    SmemLayoutdQaccum{});
    Tensor sLSEMma =
    ...
    static_assert(stride<0>(typename TiledMmaSdP::ALayout{}) == 0 and
                  ...
                  "Stride of the first mode must be 0 and the size of the mode "
                  "must be NumThreadsPerWarpGroup");
    constexpr int MmaWarpGroups =
        NumMmaThreads / cutlass::NumThreadsPerWarpGroup;
    Layout warp_group_thread_layout =
        make_layout(make_shape(Int<MmaWarpGroups>{}),
                    make_stride(Int<cutlass::NumThreadsPerWarpGroup>{}));
    Layout warp_group_thread_layout_dq =
        make_layout(make_shape(Int<NumdQWarpGroups>{}),
                    make_stride(Int<cutlass::NumThreadsPerWarpGroup>{}));
    ...
    Tensor tdSsdS =
        smem_thr_copy_PdS.partition_D(sdSt); // ((Atom,AtomNum),PIPE_M,PIPE_N)
    ...
    // PIPE), we only take the row indices.
    Tensor tLSEsLSE = thread_mma_SdP.partition_C(sLSEMma)(
        make_coord(_, _0{}, _), _0{}, _0{}, _); // (2, V, PIPE)
    Tensor tLSEsdPsum = thread_mma_SdP.partition_C(sdPsumMma)(
        make_coord(_, _0{}, _), _0{}, _0{}, _);
    ...
    auto compute_dQ = [&]() {
      ...
      cutlass::arch::NamedBarrier::sync(
          ...
          static_cast<int>(
              BwdNamedBarriers::dQEmpty) /*id*/); // sdQ empty, ready to be
                                                  // written to
      Tensor tdQrdQ =
          partition_fragment_C(tiled_mma_dQ, select < !dQ_swapAB ? 0 : 2,
      ...
      if constexpr (!dQ_swapAB) {
        ...
        flash::gemm</*zero_init=*/true, /*wg_wait=*/1>(
            tiled_mma_dQ, tdQrdS(_, _, _, smem_pipe_read.index()), tdQrK,
            tdQrdQ);
      } else {
        Tensor tdQrdS = wg_mma_dQ.partition_fragment_B(sdS);
        Tensor tdQrK = wg_mma_dQ.partition_fragment_A(sKt);
        flash::gemm</*zero_init=*/true, /*wg_wait=*/1>(
            tiled_mma_dQ, tdQrK, tdQrdS(_, _, _, smem_pipe_read.index()),
            tdQrdQ);
      }
      pipeline_q.consumer_release(smem_pipe_read); // release Q
      warpgroup_wait<0>();
      Tensor taccdQrdQ = r2s_thr_copy_dQaccum.retile_S(
          tdQrdQ); // ((Atom,AtomNum), MMA_M, MMA_N)
    ...
    if constexpr (Is_causal) {
      static constexpr int n_masking_steps =
          cute::ceil_div(kBlockN, kBlockM) + 1;
      CUTLASS_PRAGMA_NO_UNROLL
      for (; m_block < std::min(m_block_max, m_block_min + n_masking_steps);
           ++m_block) {
        Tensor tSrS =
            partition_fragment_C(tiled_mma_SdP, select<1, 0>(TileShape_MNK{}));
        pipeline_q.consumer_wait(smem_pipe_read);
        flash::gemm</*zero_init=*/true, /*wg_wait=*/-1>(
            tiled_mma_SdP, tSrK, tSrQ(_, _, _, smem_pipe_read.index()), tSrS);
        ...
        flash::gemm</*zero_init=*/true, /*wg_wait=*/-1>(
            tiled_mma_SdP, tdPrV, tdPrdO(_, _, _, smem_pipe_read.index()),
            tdPrdP);
        warpgroup_wait<1>();
        Tensor cS = cute::make_identity_tensor(select<1, 0>(TileShape_MNK{}));
        Tensor taccScS = thread_mma_SdP.partition_C(cS);
        int causal_row_offset =
            1 + seqlen_k - n_block * kBlockN - seqlen_q + m_block * kBlockM;
#pragma unroll
        for (int i = 0; i < size(tSrS); ++i) {
          if (int(get<0>(taccScS(i))) >=
              std::min(int(get<1>(taccScS(i))) + causal_row_offset,
        ...
        Tensor scores = make_tensor(
            tSrS.data(),
            flash::convert_layout_acc_transposed_rowcol(tSrS.layout()));
        flash::scale_apply_exp2</*Scale_max=*/false, /*Check_inf=*/false>(
            scores, group_modes<0, 2>(tLSErLSE), params.softmax_scale_log2);
...
#pragma unroll
        ...
        cute::copy(smem_tiled_copy_PdS, tdSadS,
...
#pragma unroll
      for (int i = 0; i < size(tSrS); ++i) {
        if (int(get<0>(taccScS(i))) >= int(seqlen_k - n_block * kBlockN)) {
          tSrS(i) = -INFINITY;
        }
      }
      // Reshape tSrS from ((2, 2, V), MMA_N, MMA_M) to (nrow=(2, V, MMA_M),
