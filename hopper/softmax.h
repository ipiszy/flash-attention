...
__device__ __forceinline__ void thread_reduce_(Tensor<Engine0, Layout0> const &tensor, Tensor<Engine1, Layout1> &summary, Operator &op, RowScaleT& row_scale) {
    ...
    if constexpr (!zero_init && !cute::is_same_v<RowScaleT, std::nullptr_t>) {
        #pragma unroll
        for (int mi = 0; mi < size<0>(tensor); mi++) {
            summary(mi) = summary(mi) / row_scale(mi);
        }
    }
    #pragma unroll
...
template<bool zero_init=true, bool warp_reduce=true, typename Engine0, typename Layout0, typename Engine1, typename Layout1, typename RowScaleT>
__device__ __forceinline__ void reduce_sum(Tensor<Engine0, Layout0> const& tensor, Tensor<Engine1, Layout1> &sum, RowScaleT &row_scale){
    SumOp<float> sum_op;
...
__forceinline__ __device__ void scale_apply_exp2(Tensor<Engine0, Layout0> &tensor, Tensor<Engine1, Layout1> const &max, const float scale, RowScaleT &row_scale) {
    ...
    for (int mi = 0; mi < size<0>(tensor); ++mi) {
        ...
        const float max_scaled = Check_inf
            ? (max(mi) == -INFINITY ? 0.f : (!Scale_max ? max(mi) : max(mi) * scale_m) - max_offset)
            : (!Scale_max ? max(mi) : max(mi) * scale_m) - max_offset;
        #pragma unroll
        for (int ni = 0; ni < size<1>(tensor); ++ni)  {
...
struct Softmax {
        ...
        if constexpr (Is_first) {
            ...
            for (int mi = 0; mi < size(row_max); ++mi) {
                ...
                if constexpr (cute::is_same_v<RowScaleT, std::nullptr_t>) {
                    ...
                    scores_scale(mi) = exp2f((scores_max_prev(mi) - scores_max_cur) * softmax_scale_log2 * row_scale(mi));
                }
    ...
    __forceinline__ __device__ void online_softmax(Tensor0 &acc_s, RowScaleT &row_scale) {
        ...
        flash::reduce_sum</*zero_init=*/Is_first, /*warp_reduce=*/false>(scores, row_sum, row_scale);
    };
