...
public:
    ...
    static constexpr uint32_t LoadRegisterRequirement = NumMmaWarpGroups == 1 ? 56 : (NumMmaWarpGroups == 2 ? ((Use_TMA_KV && CollectiveMainloop::kScalingRecipe != ScalingRecipe::PerQTokenKVBlock) ? 24 : 40) : 32);
    static constexpr uint32_t MmaRegisterRequirement = NumMmaWarpGroups == 1 ? 256 : (NumMmaWarpGroups == 2 ? ((Use_TMA_KV && CollectiveMainloop::kScalingRecipe != ScalingRecipe::PerQTokenKVBlock) ? 240 : 232) : 160);
