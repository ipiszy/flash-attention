...
__global__ void __launch_bounds__(Ktraits::kNWarps *cutlass::NumThreadsPerWarp,
    ...
    compute_attn_ws(
        CUTE_GRID_CONSTANT typename CollectiveMainloopFwd<
            Ktraits, Is_causal, Seqlen_traits>::Params const mainloop_params,
        CUTE_GRID_CONSTANT
  ...
  static constexpr int NumCopyThreads =
      !Is_WS ? 0 : cutlass::NumThreadsPerWarpGroup;
  static constexpr int kBlockM = Ktraits::kBlockM;
  // static constexpr int kBlockN = Ktraits::kBlockN;
  // constexpr int kHeadDim = Ktraits::kHeadDim;

  using CollectiveMainloop =
      CollectiveMainloopFwd<Ktraits, Is_causal, Seqlen_traits>;
  ...
  if (warp_group_idx == 0) { // Producer
    ...
    if (warp_idx_in_warpgroup == 0) { // Load Q, K, V
      ...
      for (auto work_tile_info = scheduler.get_initial_work();
        ...
        collective_mainloop.load(mainloop_params, pipeline_k, pipeline_v,
      ...
      collective_mainloop.load_tail(pipeline_k, pipeline_v, smem_pipe_write_k,
                                    smem_pipe_write_v);
    }
  } else { // Consumer
    ...
    for (auto work_tile_info = scheduler.get_initial_work();
      ...
      collective_mainloop.mma(mainloop_params, pipeline_k, pipeline_v,
                              smem_pipe_read_k, smem_pipe_read_v, tOrO, softmax,
                              n_block_max, threadIdx.x - NumCopyThreads,
                              work_idx, m_block, shared_storage,
                              seqlen_traits_q, seqlen_traits_k);
      // tOrO, softmax, n_block_max, threadIdx.x - NumCopyThreads + (work_idx >>
      // 30), work_idx, shared_storage);
...
__global__ void __launch_bounds__(Ktraits::kNWarps *cutlass::NumThreadsPerWarp,
  ...
  // for now, disable for hdim 128 causal to avoid perf regression with register
  // spilling
  static constexpr bool Use_max_offset =
      !(Is_causal && Ktraits::kHeadDim == 128);
  ...
  MainloopPipelineVt pipeline_vt(shared_storage.pipeline_vt,
                                 pipeline_params_vt);
  ...
  if (warp_group_idx == 0) { // Producer
    ...
    for (auto work_tile_info = scheduler.get_initial_work();
      ...
      if constexpr (Is_causal) {
        if (n_block_max <= 0) {
          ...
          cutlass::arch::NamedBarrier::sync(
              NumCopyThreads,
              static_cast<int>(FwdNamedBarriers::ProducerWG) /*id*/);
          continue;
        }
      }
      collective_mainloop.load_fp8(mainloop_params, pipeline_k, pipeline_v,
                                   pipeline_vt, smem_pipe_write, smem_pipe_read,
                                   shared_storage, scheduler, scheduler_params,
                                   work_tile_info, block_coord, work_idx,
                                   seqlen_traits_q, seqlen_traits_k);
      ++work_idx;
      ...
      // if constexpr (Is_causal) {
      //     cutlass::arch::NamedBarrier::sync(NumCopyThreads,
      //     static_cast<int>(FwdNamedBarriers::ProducerWG) /*id*/); }
    }
    collective_mainloop.load_tail_one_write(pipeline_k, pipeline_v,
                                            smem_pipe_write);
  } else { // Consumer
