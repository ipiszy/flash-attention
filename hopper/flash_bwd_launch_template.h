...
void run_flash_bwd(Flash_bwd_params &params, cudaStream_t stream) {
  ...
  typename PreprocessKernel::Arguments preprocess_args{
      ...
      {params.do_row_stride, _1{}, params.do_head_stride,
       !Varlen ? params.do_batch_stride : 0}, // stride_dO
      static_cast<float *>(params.dsoftmax_sum),
      {!Varlen ? params.seqlen_q_rounded : total_q_padded_rounded, params.h,
       !Varlen ? params.b : 1}, // shape_dPsum
      {_1{}, !Varlen ? params.seqlen_q_rounded : total_q_padded_rounded,
       !Varlen ? params.h * params.seqlen_q_rounded : 0}, // stride_dPsum
      static_cast<float *>(params.softmax_lse_ptr),
      {_1{}, !Varlen ? params.seqlen_q : params.total_q,
       !Varlen ? params.h * params.seqlen_q : 0}, // stride_LSE
  ...
  cutlass::device_kernel<PreprocessKernel>
  ...
  using AttnKernel =
      flash::FlashAttnBwd<CollectiveMainloop, CollectiveEpilogue, Scheduler>;
  ...
  typename CollectiveMainloop::Arguments mainloop_args{
      ...
      {params.v_row_stride, _1{}, params.v_head_stride,
       !Varlen ? params.v_batch_stride : 0}, // stride_V
      static_cast<Element const *>(params.do_ptr),
      {params.do_row_stride, _1{}, params.do_head_stride,
       !Varlen ? params.do_batch_stride : 0}, // stride_dO
      static_cast<ElementAccum *>(params.dq_accum_ptr),
      // {params.seqlen_q_rounded, params.d_rounded, params.h, params.b},  //
      // shape_dQaccum {params.d_rounded, _1{}, params.d_rounded *
  ...
  typename CollectiveEpilogue::Arguments epilogue_args{
      ...
      {params.dv_row_stride, _1{}, params.dv_head_stride,
       !Varlen ? params.dv_batch_stride : 0},
      params.cu_seqlens_k};
  ...
  // AttnKernel::SharedStorage{}).mainloop.smem_q)); int smem_size_do =
  // sizeof(decltype((typename AttnKernel::SharedStorage{}).mainloop.smem_do));
  // int smem_size_ds = sizeof(decltype((typename
  // AttnKernel::SharedStorage{}).mainloop.smem_ds)); int smem_size_dqacc =
  // sizeof(decltype((typename
  // AttnKernel::SharedStorage{}).mainloop.smem_dqacc)); int smem_size_k =
  // sizeof(decltype((typename AttnKernel::SharedStorage{}).mainloop.smem_k));
  // int smem_size_v = sizeof(decltype((typename
  // AttnKernel::SharedStorage{}).mainloop.smem_v)); printf("smem_size = %d, q =
  // %d, k = %d, v = %d, do = %d, ds = %d, dqacc = %d\n", smem_size,
  // smem_size_q, smem_size_k, smem_size_v, smem_size_do, smem_size_ds,
  // smem_size_dqacc);
  ...
  if (smem_size_postprocess >= 48 * 1024) {
    CHECK_CUDA(cudaFuncSetAttribute(postprocess_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    smem_size));
  }
  postprocess_kernel<<<grid_m_postprocess,
