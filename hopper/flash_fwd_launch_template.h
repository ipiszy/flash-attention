...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  using TileShape_MNK = typename Kernel_traits::TileShape_MNK_QK;
  using ClusterShape = typename Kernel_traits::ClusterShape_MNK;

  // print(typename Kernel_traits::SmemLayoutVt{}); printf("\n"); print(typename
  // Kernel_traits::SmemLayoutVt_tmp{});
  using CollectiveMainloop =
      flash::CollectiveMainloopFwd<Kernel_traits, Is_causal, Seqlen_traits>;
  using CollectiveEpilogue =
      flash::CollectiveEpilogueFwd<Kernel_traits, Seqlen_traits>;
  using Scheduler = std::conditional_t<
      Seqlen_traits::kUseVarSeqLen, flash::SingleTileScheduler,
      std::conditional_t<
          !Is_causal, flash::StaticPersistentTileScheduler,
  ...
  Seqlen_traits seqlen_traits_q(params.total_q, params.seqlen_q,
                                params.cu_seqlens_q);
      ...
      CollectiveMainloop::to_underlying_arguments(
          {static_cast<Element const *>(params.q_ptr),
           ...
           seqlen_traits_k.get_gmem_layout(
               params.seqlen_k, params.d_qk, params.h_k, params.b,
               params.k_row_stride, params.k_head_stride,
               params.k_batch_stride), // layout_K
           static_cast<Element const *>(params.v_ptr),
           seqlen_traits_k.get_gmem_layout(
               params.seqlen_k, params.d_vo, params.h_k, params.b,
  ...
  if constexpr (cutlass::sizeof_bits_v<Element> == 8)
    kernel = (void *)flash::compute_attn_ws_fp8<Kernel_traits, Is_causal,
                                                Scheduler, Seqlen_traits>;
  else
...
void run_mha_fwd_hdim64(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      run_flash_fwd<Flash_fwd_kernel_traits<Headdim, Headdim, 192, 128, 16, 2,
                                            false, 1, T>,
...
void run_mha_fwd_hdim128(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      ...
      // Is_causal
      BOOL_SWITCH(
          cutlass::ceil_div(params.seqlen_q, 128) % 2 == 0 && !Is_causal &&
              !Seqlen_traits::kUseVarSeqLen,
          UseCluster, [&] {
            run_flash_fwd<Flash_fwd_kernel_traits<Headdim, Headdim, 128,
                                                  Is_causal ? 128 : 176, 12, 2,
                                                  false, UseCluster ? 2 : 1, T>,
                          Is_causal, Seqlen_traits>(params, stream);
          });
    });
  });
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      ...
      BOOL_SWITCH(
          ...
          UseCluster, [&] {
            run_flash_fwd<
                Flash_fwd_kernel_traits<Headdim, Headdim, 128, 80, 12, 2, false,
                                        UseCluster ? 2 : 1, T>,
                Is_causal, Seqlen_traits>(params, stream);
          });
    });
...
template <typename T>
void run_mha_fwd_hdim192_128(Flash_fwd_params &params, cudaStream_t stream) {
  constexpr static int HeaddimQK = 192;
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      ...
      BOOL_SWITCH(
          ...
          UseCluster, [&] {
            run_flash_fwd<
                Flash_fwd_kernel_traits<HeaddimQK, HeaddimVO, 128, 112, 12, 2,
                                        false, UseCluster ? 2 : 1, T>,
                Is_causal, Seqlen_traits>(params, stream);
          });
    });
...
void run_mha_fwd_hdim64_fp8(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      // Only use Cluster if number of tiles along seqlen_q is even
      BOOL_SWITCH(
          cutlass::ceil_div(params.seqlen_q, kBlockM) % 2 == 0 && !Is_causal &&
              !Seqlen_traits::kUseVarSeqLen,
          UseCluster, [&] {
            run_flash_fwd<Flash_fwd_kernel_traits_fp8<Headdim, kBlockM, kBlockN,
                                                      kNWarps, kStages, false,
                                                      UseCluster ? 2 : 1, T>,
                          Is_causal, Seqlen_traits>(params, stream);
          });
    });
