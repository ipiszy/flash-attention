...
public:
  ...
  using GmemLayoutAtom =
      Layout<Shape<Int<MaxThreadsPerBlock / kGmemThreadsPerRow>,
  ...
  static_assert(get<1>(TileShape_MK{}) % kGmemElemsPerLoadAccum == 0,
                "Headdim must be a multiple of kGmemElemsPerLoadAccum");
  static constexpr int kGmemThreadsPerRowAccum =
      kBlockKGmem / kGmemElemsPerLoadAccum;
  static_assert(
      MaxThreadsPerBlock % kGmemThreadsPerRowAccum == 0,
      "MaxThreadsPerBlock must be a multiple of kGmemThreadsPerRowAccum");
  using GmemLayoutAtomAccum =
      Layout<Shape<Int<MaxThreadsPerBlock / kGmemThreadsPerRowAccum>,
                   Int<kGmemThreadsPerRowAccum>>,
             Stride<Int<kGmemThreadsPerRowAccum>, _1>>;
  using GmemTiledCopyAccum = decltype(make_tiled_copy(
      Copy_Atom<DefaultCopy, ElementAccum>{}, GmemLayoutAtomAccum{},
      Layout<Shape<_1, Int<kGmemElemsPerLoadAccum>>>{})); // Val layout, 4 vals
                                                          // per store
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    return {args.ptr_O,       args.shape_O,       args.stride_O,
            ...
            args.num_batch,   args.dq_semaphore,  args.cu_seqlens};
  }
  ...
  void operator()(Params const &params, [[maybe_unused]] char *smem_buf) {
    ...
    int const seqlen_o = !is_varlen ? get<0>(params.shape_O)
                                    : params.cu_seqlens[bidb + 1] - offset_o;
    if (is_varlen && m_block * kBlockM >= seqlen_o) {
      return;
    }
    ...
    Tensor mLSE =
        make_tensor(make_gmem_ptr(params.ptr_LSE), shape_LSE,
                    params.stride_LSE)(_, bidh, !is_varlen ? bidb : 0);
    Tensor gLSE = local_tile(cute::domain_offset(make_coord(offset_o), mLSE),
...
#pragma unroll
    ...
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/true>(gmem_tiled_copy_O, tOgO, tOrO, tOcO,
                                       tOpO, seqlen_o - m_block * kBlockM);
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/true>(gmem_tiled_copy_O, tOgdO, tOrdO, tOcO,
                                       tOpO, seqlen_o - m_block * kBlockM);

    // Reshape from e.g. (8, kBlockM / 32, kHeadDim / 64) to (kBlockM / 32, (8,
    // kHeadDim / 64))
    Layout l =
        make_layout(get<1>(tOrO.layout()),
...
#pragma unroll
      ...
      dP_sum(mi) =
          flash::Allreduce<kGmemThreadsPerRow>::run(dP_sum_cur, sum_op);
    }

    // If varlen, the layout for dPSum, LSE_log2, and dQaccum is that we pad
    // each sequence in the batch by an extra 128, so that the write for each
    // sequence doesn't touch the next sequence. Sequence i starts at
    // params.cu_seqlens[i] + i * 128 and ends at params.cu_seqlens[i + 1] + i *
    // 128
    int const offset_padded =
        !is_varlen ? 0 : (params.cu_seqlens[bidb] + bidb * 128) / 128 * 128;
    Tensor mdPsum =
        make_tensor(make_gmem_ptr(params.ptr_dPsum), params.shape_dPsum,
                    params.stride_dPsum)(_, bidh, !is_varlen ? bidb : 0);
    Tensor gdPsum =
        local_tile(cute::domain_offset(make_coord(offset_padded), mdPsum),
                   Shape<Int<kBlockM>>{}, make_coord(m_block));
    if (thread_idx % kGmemThreadsPerRow == 0) {
#pragma unroll
      for (int mi = 0; mi < size(dP_sum); ++mi) {
        int row = thread_idx / kGmemThreadsPerRow +
                  mi * MaxThreadsPerBlock / kGmemThreadsPerRow;
    ...
    Tensor mLSElog2 =
        make_tensor(make_gmem_ptr(params.ptr_LSE_log2), params.shape_dPsum,
                    params.stride_LSE_log2)(_, bidh, !is_varlen ? bidb : 0);
    Tensor gLSElog2 =
        local_tile(cute::domain_offset(make_coord(offset_padded), mLSElog2),
                   Shape<Int<kBlockM>>{}, make_coord(m_block));
    ...
    if constexpr (Clear_dQaccum) {
      Tensor mdQaccum =
          make_tensor(make_gmem_ptr(params.ptr_dQaccum), params.shape_dQaccum,
                      params.stride_dQaccum)(_, _, bidh, !is_varlen ? bidb : 0);
      Tensor gdQaccum = local_tile(
          cute::domain_offset(make_coord(offset_padded, _0{}), mdQaccum),
          TileShape_MK{}, make_coord(m_block, _0{}));
...
#pragma unroll
    ...
    if (params.dq_semaphore != nullptr && thread_idx == 0) {
      ...
      params.dq_semaphore[bidh + bidb * num_head +
                          m_block * num_head * num_batch] = 0;
    }
  }
};
